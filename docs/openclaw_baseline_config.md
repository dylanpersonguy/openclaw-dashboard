...
This guide turns the provided baseline into a practical starting point for local OpenClaw setup and Dashboard integration.
...
- Align this with Dashboard gateway `workspace_root` for consistency.
...
## Dashboard Connection (This Repo)

When adding a gateway in Dashboard:
...
- Workspace root (in Dashboard gateway config): align with `agents.defaults.workspace` when possible
