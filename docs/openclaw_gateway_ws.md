...
OpenClaw Dashboard supports both secure (`wss://`) and non-secure (`ws://`) WebSocket connections to gateways.
...
When enabled, Dashboard skips TLS certificate verification for that gateway connection.
